const toDoList = {
    ...
    login: function () {
        ...
        ____.currentUser = prompt("Username을 입력해 주세요!");
        let isnew = true;
        for (existing in ____.users) {
            // for in
            if (existing === ____.currentUser) isnew = false;
        }
        if (isnew) {
            alert(`${____.currentUser}님 처음 오신 것을 환영합니다.`);
            ____.users[____.currentUser] = [
...
function showList() {
    let i = 0;
    console.log("**************");
    ________.users[________.currentUser].forEach((user) => {
...
function addList() {
    ...
    ________.users[________.currentUser].push(newTask);
}

function deleteList() {
    showList();
    let index = parseInt(prompt("몇 번째 일을 삭제할까요?"));
    ________.users[________.currentUser].splice(index, 1);
}

function printCaps() {
    console.log(
        ________.users[________.currentUser]
            .filter((user) => typeof user === "string")
            .map((user) => user.toUpperCase())
    );
}

________.login(); //함수 호출
